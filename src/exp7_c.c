...
int main() {
  ...
  // Sort memory partitions in ascending order for Best Fit
  for (i = 0; i < n - 1; i++) {
    t = i;
    for (j = i + 1; j < n; j++) {
      if (mem[t] > mem[j]) {
  ...
  // Display sorted memory blocks (optional)
  printf("\nSorted Memory Partitions (Ascending):\n");
  for (i = 0; i < n; i++) {
    printf("%d ", mem[i]);
  }
  printf("\n");
  ...
  for (i = 0; i < m; i++) {
    ...
    for (j = 0; j < n; j++) {
      if (pro[i] <= mem[j]) {
        printf("\nProcess %dK is placed in memory block %dK", pro[i], mem[j]);
    ...
    if (!allocated) {
      printf("\nProcess %dK cannot be placed", pro[i]);
    }
  }
